...
public class FilmDbStorage implements FilmStorage {
    ...
    public List<Film> readBestDirectorFilms(int directorId, String param) {
        ...
        if (param.equals("likes")) {
            sqlQuery = "SELECT * FROM films f JOIN mpa m ON f.film_mpa_id = m.mpa_id RIGHT JOIN film_directors fd ON f.film_id = fd.film_id AND fd.director_id=? LEFT JOIN likes fl ON f.film_id = fl.film_id GROUP BY f.FILM_ID, fl.film_id, fl.user_id ORDER BY COUNT(fl.FILM_ID) DESC";
        } else if (param.equals("year")) {
            sqlQuery = "SELECT * FROM films f JOIN mpa m ON f.film_mpa_id = m.mpa_id JOIN film_directors fd ON f.film_id = fd.film_id AND fd.director_id=? GROUP BY f.film_id ORDER BY f.FILM_RELEASE_DATE";
        } else {
            return null;
        }
    ...
    public List<Film> getRecommendations(Integer id) {
        final String sqlQuery = "SELECT * FROM films f JOIN mpa m ON f.film_mpa_id = m.mpa_id " +
                "LEFT JOIN likes fl ON f.film_id = fl.film_id WHERE f.film_id IN (SELECT l1.film_id FROM likes l1, likes l2, likes l3 " +
                "WHERE l2.user_id = ? AND l3.film_id = l2.film_id AND l3.user_id != l2.user_id AND l1.film_id != l2.film_id AND l1.user_id = l3.user_id)" +
